...
if (hasActive) {
  ...
  return NextResponse.json({
    success: false,
    error: 'You already have an active subscription. Please cancel it in Settings before creating a new one, or contact support.',
...
if (existingStripeSubscriptionId) {
  try {
    ...
    const { hasActive } = await checkActiveSubscriptions(stripe, existingStripeCustomerId);
    if (hasActive) {
      return NextResponse.json({
        success: false,
        error: 'You have an active subscription. Please manage it in Settings.',
      }, { status: 400 });
    }
...
if (activeSubscriptions.length > 1) {
  console.error('[stripe:webhook] ALERT: Customer has multiple active subscriptions', {
    customerId: stripeCustomerId,
