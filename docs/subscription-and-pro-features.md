...
| **Bookings (V2 / availability)**      | **5** appointments **lifetime** while Free (`FREE_BOOKINGS_LIMIT`). Same `isProAccess` gate as the dashboard: cancel, `past_due`, unpaid, paused, or never-subscribed all count as Free. After 5 they cannot create or accept more (public book, owner create, quote accept, paid checkout). Tracked with **`free_bookings_count`** only. | Unlimited (cap not applied).                       |
| **Customer SMS (booking flow)**       | Same texts as Pro on those bookings: confirmation, day-before reminder, on the way, job started, work finished, and completion receipt. Volume stays small because new bookings stop at 5. Consent, opt-out, and the Telnyx master switch still apply. | Same flow, unlimited bookings.                     |
