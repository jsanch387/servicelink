...
function resolveRefreshDeepLink(): string {
  return (
    process.env.STRIPE_MOBILE_CONNECT_DEEP_LINK_REFRESH_URL?.trim() ||
    'servicelinkmobile://payments/connect?connect=refresh'
  );
}

export async function GET(request: NextRequest) {
  const deepLink = resolveRefreshDeepLink();
  const webFallback = `${request.nextUrl.origin}${ROUTES.DASHBOARD.PAYMENTS}?connect=refresh`;
  console.info(`${LOG} refresh`, {
