...
We store event IDs for every event type we process: `checkout.session.completed`, `checkout.session.expired`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_failed`. Other events are acknowledged with 200 but not stored. Create this table in Supabase:
...
2. **Connect booking checkout webhook** (`/api/stripe/webhook-connect`)  
   ...
   Events: `checkout.session.completed`, `checkout.session.expired` (create-payment links), `checkout.session.async_payment_succeeded`, `checkout.session.async_payment_failed` (and optionally `account.updated`).
