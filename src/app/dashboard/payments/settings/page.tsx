...
import { isStripeConnectReady } from '@/features/payments/utils/isStripeConnectReady';
...
export default async function DashboardPaymentsSettingsPage({
  searchParams,
}: {
  ...
  if (shouldHandleConnectQuery) {
    ...
    if (process.env.STRIPE_SECRET_KEY?.trim()) {
      ...
      if (!syncResult.ok) {
        ...
        console.error(
          'DashboardPaymentsSettingsPage: Connect sync failed',
          syncResult.error
        );
    ...
    redirect(ROUTES.DASHBOARD.PAYMENTS_SETTINGS);
  }
  ...
  const stripeConnectReady = isStripeConnectReady(paymentAccount);
