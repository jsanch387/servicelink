...
- **New subscriber:** on first `checkout.session.completed` membership upsert (`created`), owner gets in-app + push (`reference_type: subscriber`, `reference_id: customer_memberships.id` → subscriber detail). First-visit booking still also sends the usual new-appointment owner notify when that booking is created.
