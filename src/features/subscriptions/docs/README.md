...
| Area | Status |
| ---- | ------ |
| Owner create / edit / soft-delete plans | ✅ Live (DB only) |
...
| Stripe Product + Price sync | ❌ Not started (`stripe_*` columns null) |
| Checkout `mode: 'subscription'` | ❌ Stub toast on Continue |
| Customer memberships table / webhooks | ❌ Not started |
| Real subscriber count / delete block | ⚠️ Gate exists; counter stubs `0` |
...
1. Sync Stripe **Product** + **Price(s)** on create/edit (Connect account); store IDs.
...
4. Customer Portal for cancel / payment method.
