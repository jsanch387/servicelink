...
/**
 * Whether this business's owner is in the Booking Link 2.0 email rollout.
 * Uses admin client (auth.admin + business_profiles).
 */
export async function isBusinessInBookingLinkV2Rollout(
  ...
  try {
    ...
    return isOwnerEmailAllowedForBookingLinkV2Rollout(email);
  } catch {
    return false;
  }
}
