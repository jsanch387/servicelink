...
async function limit(
  ...
  return memory.allow(`account_delete:${key}`, 1, WINDOW_MS);
}

export async function assertDeleteAccountRateLimit(
  request: NextRequest,
  userId: string
): Promise<
  { ok: true } | { ok: false; retryAfterSec: number; reason: 'user' | 'ip' }
> {
  const userLimiter = getLimiter(userLimiterCache, 'account_delete:user');
  ...
  const ipLimiter = getLimiter(ipLimiterCache, 'account_delete:ip');
