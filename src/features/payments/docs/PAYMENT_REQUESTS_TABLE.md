...
- Status: implemented for **payment links** (v1)
...
| `collection_method`            | `text`        |       no | `checkout_link` now; `tap_to_pay` later            |
...
| `stripe_payment_intent_id`     | `text`        |      yes | Set when Checkout completes                        |
