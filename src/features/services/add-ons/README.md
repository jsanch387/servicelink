...
| `description`      | text, nullable    | **Optional**, **customer-facing** detail about what the add-on includes. Empty input is stored as `null`. Capped at `ADD_ON_DESCRIPTION_MAX_LENGTH` (800) in the UI, on the server, and by the `service_addons_description_length_check` constraint — **all three must stay in sync**. Shown only in the public book flow (behind a "See description" toggle), not in the owner's add-on list. |
